...
def get_jira_credentials():
    ...
    if not JIRA_USERNAME:
        JIRA_USERNAME = input("JIRA_USERNAME environment variable is not set. Please enter your JIRA username:")
...
def main():
    ...
    pr_title = pr["title"]
    commit_title = input("Commit title [%s]: " % pr_title.encode("utf-8"))
    if commit_title == "":
    ...
    if JIRA_IMPORTED:
        ...
        print("Could not find jira-python library. Run 'pip install jira[opt,cli,testing]' to install.")
        print("Exiting without trying to close the associated JIRA.")
