#!/usr/bin/perl
# Check given Ivory job status
# Author: Gamini Kiran Praneeth (kiran.praneeth@inmobi.com)
...
$httphost_url = "http://$ooziehost/ivory/api/processinstance/status/$process?start=$start&end=$end";
$query = "curl -s -H'remote-user: user'"." \"$httphost_url\"";
$query_output = `$query` || die "Curl call failed: $!";

if ($query_output eq "") {
    print "Null output. Check if Ivory is down or jobs are hung from long time\n";
    exit 2;
}
...
foreach (@$instances_ref) {
    ...
    if ($instance_ref->{status} eq "FAILED" || $instance_ref->{status} eq "KILLED" || $instance_ref->{status} eq "SUSPENDED") {
        ...
        # 2012-06-05T06:40Z
        my @ivory_output = `ivory_job_status.pl --proc $process -jid $instance_id_raw`;
        if ($? != 0 ) {
           $alert_string = "CRITICAL: Failed running ivory_job_status.pl --proc $process -jid $instance_id";
           push (@running, $alert_string);
        }
        next if !$ivory_output[0];
        my $jobruntime;
        if ($ivory_output[0] =~ /last(.*)seconds/ ) {
            $jobruntime = $1;
        }
        $jobruntime =~ s/\s+//g;
...
Check the status of an Ivory job in a given window and report it to Nagios 

=head1 SYNOPSIS

 Usage: check_ivory_job.pl [options]
...
$ perl check_ivory_job.pl --proc download-summary
Succeded:2012-05-28T05:40Z 2012-05-28T06:40Z
gaminik@proc2000:/opt/mkhoj/ops/lib/nrpe

$ perl check_ivory_job.pl --proc <processname> --start 2012-05-28T01:40Z --end 2012-05-28T06:40Z
Succeded:2012-05-28T01:40Z 2012-05-28T02:40Z 2012-05-28T03:40Z 2012-05-28T04:40Z 2012-05-28T05:40Z 2012-05-28T06:40Z
$ 

$ perl check_ivory_job.pl --proc <processname> --start 2012-05-28T07:40Z --end 2012-05-28T08:40Z
UNKNOWN STATE
$

$ perl check_ivory_job.pl --proc <processname> --start 2012-05-28T03:40Z --end 2012-05-28T07:40Z --threshold 10
...
$perl check_ivory_job.pl --proc <processname> --start 2012-05-28T03:40Z --end 2012-05-28T07:40Z --threshold 100
