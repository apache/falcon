...
(function () {
  ...
  dashboardCtrlModule.controller('DashboardCtrl', [ "$scope", "Falcon", "EntityModel", "FileApi", "$state", "X2jsService", "$window",
    function ($scope, Falcon, EntityModel, FileApi, $state, X2jsService, $window) {
      ...
      $scope.entityDetails = function (name, type) {
        type = type.toLowerCase(); //new sandbox returns uppercase type
        $window.location.href = "/entity.html?type=" + type + "&id=" + name + '&user.name=ambari-qa';
      };
      //----------------------------------------//
