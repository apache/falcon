...
(function () {
  ...
  clusterModule.controller('ClusterFormCtrl', [ "$scope", "$interval", "Falcon", "EntityModel", "$state", "X2jsService", 
  function($scope, $interval, Falcon, EntityModel, $state, X2jsService) {
    ...
    function normalizeModel() {
      ...
      //--------------TAGS--------------//      
      if($scope.clusterEntity.clusterModel.cluster.tags === "" || $scope.clusterEntity.clusterModel.cluster.tags === undefined) {
      ...
      //-------------ACL----------------//
      if(!$scope.clusterEntity.clusterModel.cluster.ACL) { $scope.clusterEntity.clusterModel.cluster.ACL = { _owner: "", _group: "", _permission: "" }; }
      //------------Location------------//
    ...
    function cleanModel() {    
      ...
      if($scope.clusterEntity.clusterModel.cluster.properties.property[lastOne]._name === "" || 
      ...
      if($scope.clusterEntity.clusterModel.cluster.ACL && $scope.clusterEntity.clusterModel.cluster.ACL._owner === "") { delete $scope.clusterEntity.clusterModel.cluster.ACL; }
      //deletes tags if empty
      if($scope.clusterEntity.clusterModel.cluster.tags.length === 0){ delete $scope.clusterEntity.clusterModel.cluster.tags;}
