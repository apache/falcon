  ...
  app.controller('RootCtrl', [ "$scope", "$timeout", "Falcon", "FileApi", "EntityModel", "$state", "X2jsService", "ValidationService",
    function ($scope, $timeout, Falcon, FileApi, EntityModel, $state, X2jsService, validationService) {
      ...
      // $scope.$watch(function() {
        // return $scope.cloningMode;
      // }, function() {
        // alert($scope.cloningMode);
      // });
      ...
      $scope.refreshList = function (type) {
        ...
        Falcon.getEntities(type)
          .success(function (data) {
            ...
            if (data === null) { $scope.lists[type + 'List'] = []; }
            else if (typeOfData === "[object Array]") { $scope.lists[type + 'List'] = data.entity; }
            else if (typeOfData === "[object Object]") { $scope.lists[type + 'List'][0] = data.entity; }
