...
(function () {
  ...
  var falconModule = angular.module('app.services.falcon', ['app.services.x2js']);
  ...
  falconModule.factory('Falcon', ["$http", "X2jsService", function ($http, X2jsService) {
    ...
    function add_user(url) {
      ...
      url = url + paramSeparator + 'user.name=ambari-qa';
      return url;
    }
    //-------------Server RESPONSE----------------------//
    Falcon.responses = {
   ...
   // serverResponse: null,
    ...
    Falcon.getEntities = function (type) {
      return $http.get(add_user('../api/entities/list/' + type + '?fields=status,tags&numResults=' + NUMBER_OF_RESULTS));
    };
